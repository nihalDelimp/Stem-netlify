...
const UnlockPopUp = ( props ) => {
    ...
    const { week_number, lesson_locked, id, class_code ,course_code } = lessonData
    ...
    const unlockLessonHandle = async () => {
        if ( checked ) {
        setLoading(true)
        await dispatch(unlockedLesson( id ,{ class_code: class_code , week_number : week_number ,course_code }))
            .then(
                response => {
                    setLoading(false)
